# BuddyGPO_EntraIntune - v0.13 by jm@cloudware.host

# --- Dependency Check (Optional) ---
if (!(Get-Module -ListAvailable -Name GroupPolicy) -or !(Get-Module -ListAvailable -Name ActiveDirectory)) {
    Write-Warning "Required modules (GroupPolicy, ActiveDirectory) not found. Script may fail."
    Write-Warning "Install via: Install-Module -Name GroupPolicy, ActiveDirectory -Force"
    # Uncomment next line to exit early if modules are critical
    # exit /exit 1
}
...
function Create-HybridJoinGPO {
    ...
    $HybridJoinSettings = @(
        @{ Name = "AutoWorkplaceJoin"; Type = "DWord"; Value = ,
...
function Create-MDMAutoEnrollGPO {
    ...
    New-GPLink -Name $GPOName -Target $OUPath -Enforced Yes
    $GroupName = "Domain Computers" # Changed to Domain Computers for broader application, if desired.
...
function Create-WinRMGPO {
    ...
    Write-Host "Configured 'AllowRemoteShellAccess' in '$GPOName'."
    Set-GPRegistryValue -Name $GPOName -Key "HKLM\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client" -ValueName "AllowUnencryptedTraffic" -Type DWord -Value 1
    Write-Host "Configured 'AllowUnencryptedTraffic' in '$GPOName'."
    Set-GPRegistryValue -Name $GPOName -Key "HKLM\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client" -ValueName "TrustedHosts" -Type String -Value "*"
    Write-Host "Configured 'TrustedHosts' in '$GPOName'."
